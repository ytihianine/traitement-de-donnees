...
def trouver_meilleurs_binomes(df: pd.DataFrame) -> Mapping[str, pd.DataFrame]:
    ...
    df_mentores_non_apparies = pd.DataFrame(
        data=df_mentores.loc[mentors_disponibles, cols_to_keep]
    )
    df_mentors_non_apparies = pd.DataFrame(
        data=df_mentors.loc[mentores_disponibles, cols_to_keep]
    )
...
def generer_rapport(dfs: Mapping[str, pd.DataFrame]) -> str:
    ...
    rapport.append(f"Mentorés non appariés: {len(dfs["df_mentors_non_apparies"])}")
    ...
    if len(df_binomes) > 0:
        ...
        rapport.append(
            f"\nBinômes avec score parfait (1800): {len(df_binomes[df_binomes['score_total'] == 1800])}"
        )
        rapport.append(
            f"Binômes avec catégorie OK ([1000: 1800[): {len(df_binomes[df_binomes['score_categorie'] == 1000])}"
        )
