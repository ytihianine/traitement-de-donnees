...
from src.infra.file_handling.local import LocalFS
from src.infra.file_handling.exceptions import (
    FileNotFoundError,
)
...
class TestMove:
    def test_move_file(self, handler: LocalFS) -> None:
        handler.write("src.txt", b"data")
        handler.move("src.txt", "dst.txt")
        assert not handler.exists("src.txt")
        assert handler.read("dst.txt").read() == b"data"

    def test_move_creates_dest_dir(self, handler: LocalFS) -> None:
        handler.write("src.txt", b"data")
        handler.move("src.txt", "sub/dst.txt")
...
class TestCopy:
    def test_copy_file(self, handler: LocalFS) -> None:
        handler.write("src.txt", b"data")
        handler.copy("src.txt", "dst.txt")
        assert handler.exists("src.txt")
        assert handler.read("dst.txt").read() == b"data"

    def test_copy_creates_dest_dir(self, handler: LocalFS) -> None:
        handler.write("src.txt", b"data")
        handler.copy("src.txt", "sub/dst.txt")
        assert handler.read("sub/dst.txt").read() == b"data"
