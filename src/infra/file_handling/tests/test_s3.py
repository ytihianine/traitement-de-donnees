...
from src.infra.file_handling.s3 import S3FS
from src.infra.file_handling.exceptions import FileHandlerError, FileNotFoundError
...
class TestMove:
    def test_move(self) -> None:
        h = _make_handler({"src.txt": b"data"})
        h.move("src.txt", "dst.txt")
        assert h.exists("src.txt") is False
...
class TestCopy:
    def test_copy(self) -> None:
        h = _make_handler({"src.txt": b"data"})
        h.copy("src.txt", "dst.txt")
        assert h.exists("src.txt") is True
        assert h.read("dst.txt").read() == b"data"
